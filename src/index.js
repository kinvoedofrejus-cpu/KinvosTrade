...
// En cas d'échec, retourne une liste vide (le bot n'est pas bloqué par une panne du calendrier).
...
async function listTrades(env, from, to) {
  ...
  // pas les simples signaux envoyés mais jamais pris.
  const conditions = [`status IN ('taken', 'closed')`];
  const params = [];
  if (from) {
...
async function runAnalysis(env) {
  ...
  const newsEvents = await fetchHighImpactEvents();
  if (isInNewsBlackout(Date.now(), newsEvents)) {
